...
import { fromDateKey, generateRoutineTodos } from '@/utils/routineSchedule';
...
export default function EditRoutineScreen({ navigation, route }: any) {
  ...
  const routine = useRoutineStore((state) => state.routines.find((item) => item.id === routineId));
  ...
  const updateRoutineTodos = useTodoStore((state) => state.updateRoutineTodos);
  const removeRoutineTodos = useTodoStore((state) => state.removeRoutineTodos);
  const removeFutureRoutineTodos = useTodoStore((state) => state.removeFutureRoutineTodos);
  ...
  useEffect(() => {
    ...
    const routineStartDate = fromDateKey(routine.startDate);
    ...
    setStartDate(routineStartDate);
    setEndDateEnabled(Boolean(routine.endDate));

    if (routine.endDate) {
      setEndDate(fromDateKey(routine.endDate));
      return;
    }

    const nextMonth = new Date(routineStartDate);
    nextMonth.setMonth(nextMonth.getMonth() + 1);
    setEndDate(nextMonth);
  }, [routine]);

  const isSaveDisabled = !title.trim() || !routine;
  ...
  const handleSave = () => {
    ...
    updateRoutine(routine.id, changes);
    updateRoutineTodos(routine.id, {
      title: trimmedTitle,
      icon: selectedIcon,
      color: selectedColor,
    });
    ...
    removeFutureRoutineTodos(routine.id, toDateKey(rangeStart));

    const generatedTodos = generateRoutineTodos(updatedRoutine, rangeStart, rangeEnd);
              ...
              <View className="ml-4 min-w-0 flex-1">
                ...
                <View className="mt-2 flex-row items-center">
                  <Text
                    ...
                    지금까지 {routine?.completedDates.length ?? 0}회 완료
                  </Text>
                </View>
              </View>
